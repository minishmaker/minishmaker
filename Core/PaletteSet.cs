...
namespace MinishMaker.Core
{
    public class PaletteSet
    {
        ...
        public PaletteSet(int pnum)
        {
            ...
            byte[] pdata = r.ReadBytes(amount * 0x20, palAddr);
            var path = $"{Project.Instance.ProjectPath}/Global/{DataType.palette}/{DataType.palette}{pnum.Hex(2)}.json";
            if (File.Exists(path))
            {
                pdata = DataHelper.GetByteArrayFromJSON(path, 2);
            }
            ...
            for (int p = start; p < (start + amount); p++)
            {
                if (p >= 16)
                {
        ...
        public string ToPaletteString()
        {
            ...
            {
                sb.Append("0x00" + color.B.Hex(2) + color.G.Hex(2) + color.R.Hex(2) + " ");
                i++;
                if (i == 4)
                {
                    sb.AppendLine();
